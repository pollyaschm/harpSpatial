...
# not exported
spatial_score_table <- function(colnames = "value") {
...
#  score_fields <- structure(rep("REAL", length(score_names)), names=score_names)
  score_fields <- rep("REAL", length(colnames))
  names(score_fields) <- colnames
  list(
    fields = c(standard_fields, score_fields),
    primary = c(names(standard_fields))
  )
}
